...
const Nav = () => {
  ...
  return (
    <div id="main">
      <div id="nav" className={`w-full max-w-full h-[15vh] md:h-[15vh] flex justify-between z-50 items-center px-4 md:px-8 fixed ${isScrolled ? 'bg-black' : 'bg-transparent'}`}>
        <div id="logo">
          <h1 className={`text-sm font-mono px-4 py-2 border-2 ${isScrolled ? 'text-white border-white' : 'text-black border-black'}`}>
            StudyPathFinder<span className="animate-ping">_</span>
          </h1>
        </div>

        <div id="infoPanel" className={`w-[50%] h-[50%] justify-around items-center rounded-full hidden md:flex border-2 ${isScrolled ? 'text-white border-white' : 'text-black border-black'}`}>
